...
class TarikdataController extends Controller
{
    ...
    public function indexkkpd(Request $request)
    {
        $userId = Auth::guard('web')->user()->id;
        $data = array(
            'title'             => 'Tarik Data SP2D KKPD SIPD RI',
            'active_side_tarik' => 'active',
            'active_tarikgu'    => 'active',
            'page_title'        => 'Pengaturan',
            'breadcumd1'        => 'Kelola User',
        ...
        return view('Tarik_data.Pajakkkpd', $data);
    }
    ...
    public function save_json(Request $request)
    {
        try {
            ...
            // Cek SP2D sudah ada atau belum
            $nomor_sp2d = $dt["ls"]["header"]["nomor_sp_2_d"] ?? '';
            if (Sp2dModel::where('nomor_sp2d', $nomor_sp2d)->exists()) {
                return redirect()->back()->with('error', 'SP2D sudah ada.');
            }

            // --- Fungsi bantu untuk normalisasi angka ---
            $normalizeNumber = function ($value) {
                if ($value === null || $value === '') return 0;
                // Hapus semua spasi, titik ribuan, ubah koma jadi titik
                $value = str_replace([' ', '.'], '', $value);
                $value = str_replace(',', '.', $value);
                return (float) $value;
            };

            // Simpan data header SP2D
            ...
            $datasp2d->nomor_sp2d = $nomor_sp2d;
            $datasp2d->tanggal_sp2d = isset($dt["ls"]["header"]["tanggal_sp_2_d"]) 
                ? Carbon::parse($dt["ls"]["header"]["tanggal_sp_2_d"])->format('Y-m-d')
                : null;
            ...
            // 🔥 Normalisasi nilai SP2D agar tidak minus
            $datasp2d->nilai_sp2d = $normalizeNumber($dt["ls"]["header"]["nilai_sp2d"] ?? 0);

            $datasp2d->nomor_spm = $dt["ls"]["header"]["nomor_spm"] ?? '';
            $datasp2d->tanggal_spm = isset($dt["ls"]["header"]["tanggal_spm"]) 
                ? Carbon::parse($dt["ls"]["header"]["tanggal_spm"])->format('Y-m-d')
                : null;
            ...
            // Simpan data detail belanja
            foreach ($detail_belanja as $row) {
                DB::table('belanja1')->insert([
                    'norekening' => $row["kode_rekening"] ?? '',
                    'uraian' => $row["uraian"] ?? '',
                    'nilai' => $normalizeNumber($row["jumlah"] ?? 0),
                    'id_sp2d' => $nomoracak
                ]);
            }

            // Simpan data potongan pajak
            if (!empty($pajak_potongan)) {
                foreach ($pajak_potongan as $row1) {
                    DB::table('potongan2')->insert([
                        'jenis_pajak' => $row1["nama_pajak_potongan"] ?? '',
                        'ebilling' => $row1["id_billing"] ?? '',
                        'nilai_pajak' => $normalizeNumber($row1["nilai_sp2d_pajak_potongan"] ?? 0),
            ...
            return redirect()->back()->with('success', 'Data SP2D berhasil disimpan!');
        } catch (\Exception $e) {
    ...
    public function save_jsongu(Request $request)
    {
        ...
        public function save_jsonkkpd(Request $request)
    {
        // Validasi input
        $nomoracak = Str::random(10);

        $datasp2d = $request->input('jsontextareakkpd');
        $dt = json_decode($datasp2d, true);

        $detail = $dt["detail"];
        
        $ceksp2d = Sp2dModel::where('nomor_sp2d', $dt["nomor_sp_2_d"])->count();
        if($ceksp2d > 0)
        {
            return redirect()->back()->with('error', 'SP2D Sudah Ada');
        }
            ...
            $datasp2d->idhalaman = $nomoracak;
            // $datasp2d->jenis = $dt["jenis"];
            $datasp2d->tahun = $dt["tahun"];
            $datasp2d->nomor_rekening = $dt["nomor_rekening"];
            $datasp2d->nama_bank = $dt["nama_bank"];
            $datasp2d->nomor_sp2d = $dt["nomor_sp_2_d"];
            $datasp2d->tanggal_sp2d = Carbon::Parse($dt["tanggal_sp_2_d"])->format('Y-m-d');
            $datasp2d->nama_skpd = $dt["nama_skpd"];
            $datasp2d->keterangan_sp2d = $dt["keterangan_sp2d"];
            $datasp2d->nilai_sp2d = $dt["nilai_sp2d"];
            $datasp2d->nomor_spm =  $dt["nomor_spm"];
            $datasp2d->tanggal_spm = Carbon::Parse( $dt["tanggal_spm"])->format('Y-m-d');
            $datasp2d->nama_ibu_kota = $dt["nama_ibu_kota"];
            $datasp2d->nama_bud_kbud = $dt["nama_bud_kbud"];
            $datasp2d->jabatan_bud_kbud = $dt["jabatan_bud_kbud"];
            $datasp2d->nip_bud_kbud = $dt["nip_bud_kbud"];
            $datasp2d->save();
